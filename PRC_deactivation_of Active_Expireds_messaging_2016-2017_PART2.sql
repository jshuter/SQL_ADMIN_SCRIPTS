...
declare @commit				int = 1 ; -- 1 to COMMIT - 0 to ROLLBACK 
...
if @debug = 1
	...
	set @msg_limit = 1000 -- starting with just 10 
		...
		BEGIN TRY 		
			...
			if @flag_send_email = 1 begin 
				...
				EXECUTE msdb.dbo.sp_send_dbmail  @profile_name = @msg_engine,
					@recipients = @td_email_address, -- 'jeffrey.shuter@scouts.ca;jshuter@teradocs.com', --;pjohnsen@scouts.ca',  --- @email_address,
